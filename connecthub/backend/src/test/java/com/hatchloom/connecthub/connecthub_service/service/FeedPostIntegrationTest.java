...
class FeedPostIntegrationTest {
        ...
        void setup() {
                ...
                testUser = new BaseUser(UUID.fromString("00000000-0000-0000-0000-000000000001"), "Test User",
                                "Test@gmail.com");
        ...
        void testCreateFeedPost() throws Exception {
                PostCreationRequest dto = new PostCreationRequest(
                                new BasePostRequest("Test Post 1", "This is a test post"), "share");
        ...
        void createFeedPost_MissingTitle() throws Exception {
                PostCreationRequest dto = new PostCreationRequest(new BasePostRequest("", "This is a test post"),
                                "share");
        ...
        void createFeedPost_DifferentTypes() throws Exception {
                ...
                for (String post : postTypes) {
                        PostCreationRequest dto = new PostCreationRequest(
                                        new BasePostRequest("Test " + post, "This is a test " + post), post);
        ...
        void createFeedPost_InvalidPostType() throws Exception {
                PostCreationRequest dto = new PostCreationRequest(
                                new BasePostRequest("Test Invalid", "This is a test post with invalid type"),
                                "invalidType");
        ...
        void createFeedPost_NullPostType() throws Exception {
                PostCreationRequest dto = new PostCreationRequest(
                                new BasePostRequest("Test Null Type", "This is a test post with null type"), null);
        ...
        void deleteFeedPost() throws Exception {
                PostCreationRequest dto = new PostCreationRequest(
                                new BasePostRequest("Test Post to Delete", "This post will be deleted"), "share");
                mockMvc.perform(post("/api/feed")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(dto))
                                .with(csrf())
        ...
        void deleteFeedPost_InvalidUser() throws Exception {
                ...
                mockMvc.perform(delete("/api/feed/{postId}", post.getId())
                                ...
                                .andExpect(status().isBadRequest())
                                .andExpect(content().string("User " + (unauthorizedUserId)
                                                + " is not authorized to delete post " + post.getId()));
        ...
        void testGetFeedPostsWithPagination() throws Exception {
                for (int i = 1; i <= 45; i++) {
                        PostCreationRequest dto = new PostCreationRequest(
                                        new BasePostRequest("Test Post " + i, "This is test post number " + i),
                                        "share");
                        mockMvc.perform(post("/api/feed")
                                        .contentType(MediaType.APPLICATION_JSON)
                                        .content(objectMapper.writeValueAsString(dto))
        ...
        void testGetPostsMultiplePages() throws Exception {
                ...
                while (true) {
                        ...
                        CursorResponse<FeedPostResponse> cursorResponse = objectMapper.readValue(response,
                                        objectMapper.getTypeFactory().constructParametricType(CursorResponse.class,
                                                        FeedPostResponse.class));
                        List<FeedPostResponse> posts = cursorResponse.getData();
                        for (FeedPostResponse post : posts) {
                                Assertions.assertTrue(post.id() > 0);
                                Assertions.assertTrue(post.title().startsWith("Test Post "));
                                Assertions.assertTrue(post.content().startsWith("This is test post number "));
                                Assertions.assertEquals(testUser.id, post.author());
                                totalFetched++;
                        }
